...
def labse_embedding_model(chunked_documents, model, lang_map):
...
def xlm_embedding_model(chunked_documents, model, lang_map):
    vector_stores = {chunk_type: [] for chunk_type in chunk_types}        
    for doc_name, chunks in chunked_documents.items():
        lang = lang_map.get(doc_name)
        source_link = source_links.get(doc_name, "")
...
def multilingual_embedding_model(chunked_documents, model, lang_map):
    vector_stores = {chunk_type: [] for chunk_type in chunk_types}        
    for doc_name, chunks in chunked_documents.items():
        lang = lang_map.get(doc_name)
        source_link = source_links.get(doc_name, "")
